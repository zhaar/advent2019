...
lengthUntilCoord :: Coordinate -> Coordinate -> [Direction] -> Int -> Int
lengthUntilCoord target coord [] acc = acc
lengthUntilCoord target coord ((MkDir 0 a) : ds) acc = lengthUntilCoord target coord ds acc
lengthUntilCoord target coord ((MkDir n a) : ds) acc =
  let newCoord = moveOne a coord in
      if newCoord == target
        then acc + 1
        else lengthUntilCoord target newCoord (MkDir (n-1) a : ds) (acc + 1)


lengthForIntersection :: [Direction] -> [Direction] -> Coordinate -> Int
lengthForIntersection dirs1 dirs2 inter =
  let firstPath = lengthUntilCoord inter (0,0) dirs1 0
      secondPath = lengthUntilCoord inter (0,0) dirs2 0 in
      firstPath + secondPath

program2 :: String -> Int
program2 input = let (dir1, dir2) = parseInput input
                     grid1 = constructGrid (0, 0) dir1 empty
                     grid2 = constructGrid (0, 0) dir2 empty
                     common = intersection grid1 grid2
                     lengths = fmap (lengthForIntersection dir1 dir2) (toList common) in
                     head $ sort lengths
...
main = do args <- getArgs
          case args of
               [input] -> putStrLn $ show $ program2 input
